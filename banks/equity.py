...
class EquityBank(BaseBank):
    ...
    def deposit(self, account_number: str, amount: float, description: str = "Cash deposit") -> Transaction:
        ...
        print(f"[Equity] Deposited KES {amount:,.2f} to {account_number}. Balance: KES {account.balance:,.2f}")
        return txn
    ...
    def withdraw(self, account_number: str, amount: float, description: str = "Cash withdrawal") -> Transaction:
        ...
        print(f"[Equity] Withdrew KES {amount:,.2f} from {account_number}. Balance: KES {account.balance:,.2f}")
        return txn
    ...
    def transfer(self, from_account: str, to_account: str, amount: float) -> Transaction:
        ...
        print(f"[Equity] Transferred KES {amount:,.2f}: {from_account} -> {to_account}. Fee: KES {fee}")
        return txn
