...
class KCBBank(BaseBank):
    ...
    def __init__(self):
        super().__init__(
            bank_name="KCB Bank Kenya", bank_code="01", swift_code="KCBLKENX"
        )
    ...
    def open_account(
        ...
        if customer.national_id in self._customers:
            raise ValueError(
                f"Customer {customer.national_id} already registered at KCB"
            )
    ...
    def deposit(
        ...
        print(
            f"[KCB] Deposited KES {amount:,.2f} to {account_number}. Balance: KES {account.balance:,.2f}"
        )
        return txn
    ...
    def withdraw(
        ...
        print(
            f"[KCB] Withdrew KES {amount:,.2f} from {account_number}. Balance: KES {account.balance:,.2f}"
        )
        return txn
    ...
    def transfer(
        ...
        print(
            f"[KCB] Transferred KES {amount:,.2f}: {from_account} -> {to_account}. Fee: KES {fee}"
        )
        return txn
